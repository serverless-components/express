...
function binaryCase(string, number, options) {
  ...
  if (!options.hasOwnProperty('allowOverflow')) {
    options.allowOverflow = true
  }

  if (number > binaryCase.maxNumber(string) && !options.allowOverflow) {
    return false
  }
...
binaryCase.iterator = function(string, options) {
  ...
  return {
    next: function() {
      return index > max ? { done: true } : { done: false, value: getBinaryCase(string, index++) }
    }
  }
}
...
binaryCase.maxNumber = function(string) {
  const pow = string.match(/[a-z]/gi).length
